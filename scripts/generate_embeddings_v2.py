...
def generate_embedding(text, base_url, model, api_key):
...
def main():
    ...
    api_key = os.getenv("LLM_API_KEY", "sk-omniroute-test")
    ...
    for idx, (product_id, name, slug, category, description, coverage_details) in enumerate(products, 1):
        print(f"[{idx}/{len(products)}] Processing: {name}")
        ...
        try:
            # Generate embedding
            embedding = generate_embedding(search_text, llm_base_url, embeddings_model, api_key)
